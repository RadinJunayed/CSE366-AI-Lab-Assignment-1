...
class Environment:
    ...
    def generate_tasks(self, count):
        ...
        for task_number in range(1, count + 1):
            while True:
                ...
                if location not in tasks and location != (0, 0):
    ...
    def generate_random_locations(self, count, exclude=set()):
        ...
        while len(locations) < count:
            location = (random.randint(0, self.columns - 1), random.randint(0, self.rows - 1))
            if location not in exclude and location != (0, 0):
    ...
    def get_path_difficulty(self):
        total_cells = self.columns * self.rows
        barrier_count = len(self.barrier_locations)
        return (barrier_count / total_cells) * 100